...
module.exports = (app) => {
  ...
  app.head("/v1/user/:username", async (req, res) => {
    ...
    let user = await app.redisClient.get(`USER:${req.params.username.toLowerCase()}`);
    if (!user) {
      // if not found, search database
      user = await app.models.User.findOne({
        username: req.params.username.toLowerCase(),
      });
      // Set cache key/value with 120s expiration
      await app.redisClient.set(
        `USER:${data.username}`,
        JSON.stringify(user),
        { EX: 120 }
      );
      console.log("Using Mongo data + setting cache");
    } else {
  ...
  app.get("/v1/user/:username", async (req, res) => {
    ...
    if (!user) {
      ...
      user = await new app.models.User(JSON.parse(user))
        .populate("games");
      console.log("Using cache data");
    }
   ...
   */
  app.put("/v1/user/:username", async (req, res) => {
    console.log(req.params.username)
    console.log(req.session.user.username)
    if (!req.session.user || req.params.username.toLowerCase() !== req.session.user.username)
