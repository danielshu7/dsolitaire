...
module.exports = (app) => {
  ...
  app.post("/v1/game", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        await app.redisClient.del(`USER:${req.session.user.username}`);
   ...
   *
   * @param (req.params.id} Id of game to fetch
   * @param {req.body.status} status to update to
   *
   * @return {200} game.active
   */
  app.put("/v1/game/:id/status", async (req, res) => {
    try {
      let game = await app.models.Game.findById(req.params.id);
      if (!game) {
        res.status(404).send({ error: `unknown game: ${req.params.id}` });
        ...
        try {
          await game.save();
          res.status(200).send({ status: game.active });
        }
        catch (err) {
          console.log(`Game save error: ${err}`);
          res.status(400).send({ error: `game save error occurred` });
        }
      }
    } catch (err) {
      console.log(`Game.get failure: ${err}`);
